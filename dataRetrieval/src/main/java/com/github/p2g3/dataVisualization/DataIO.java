...
public class DataIO {
	...
	/*public void createArraySpark(Dataset<Row> data){
		String[] headers = data.columns();
		DataIO.xTitle = headers[0];
		DataIo.yTitle = headers[1];

		Dataset<Row> data2 = data
            .withColumn(headers[0], selectedCategoryPre.col(headers[0]).cast(DataTypes.DoubleType))
            .withColumn(headers[1], selectedCategoryPre.col(headers[1]).cast(DataTypes.DoubleType))
            .cache();

		x = data2.select(headers[0]).takeAsList(int n);
		y = data2.select(headers[1]).takeAsList(int n)
	}*/
